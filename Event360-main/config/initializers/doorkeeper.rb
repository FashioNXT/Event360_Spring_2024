...
Doorkeeper.configure do
  ...
  resource_owner_authenticator do
    ...
    if current_user
      ...
      redirect_to users_url(return_to: request.original_url)
    end
  end
  ...
  admin_authenticator do
    current_user = User.find_by(id: session[:user_id])
    admin_user = current_user && current_user.user_type == "admin"
    if admin_user
      current_user || redirect_to(dashboard_url)
    else
  ...
  # If you are planning to use DoorkeeConfigure Doorkeeper by modifying the config/initializers/doorkeeper.rb file. Some common configurations include:per in Rails 5 API-only application, then you might
  ...
  # Allows to customize OAuth grant flows that +eachhttp+ application support.
